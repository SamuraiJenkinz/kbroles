...
| 012 | Tier 3 word-subset allowlist fallback + chip `auth-02` text realignment. Phase B data after Quick 011: 3/10 failures, ALL on chip auth-02 ("What's the naming convention and article structure?"), all `allowlist_violation` with `class: names`, `token_count: 1`. Source-corpus grep: `"article structure"` doesn't appear in any casing; source uses `"Article Body"` for the same concept. Claude was echoing the chip's terminology back as title-case `"Article Structure"` — a bigram absent from source that Tier 2's substring fallback couldn't match. Two-layer fix: (a) reword chip to `"What's the naming convention and article body format?"` (source-aligned vocabulary, prevents the paraphrase in the first place); (b) add Tier 3 word-subset check — after Tier 1+2 miss, split the bigram on whitespace and verify each constituent word appears as substring of the lowercased corpus. Allows `"Article Structure"` (both `article`+`structure` in source separately) and `"Service Now"` (compound-word split — both halves substrings of `"ServiceNow"`). CORP-02 invariant preserved via `.every()` short-circuit: `"Jane Doe"` fails because `"jane"` is genuinely absent. 4 new tests. 784/784 tests (+4). Projected pass rate: 70% → 90%+ on chip auth-02. | 2026-05-12 | 288c60c | [012-allowlist-word-subset-and-chip-realignment](./quick/012-allowlist-word-subset-and-chip-realignment/) |

---

*Last activity: 2026-05-12 — Quick 012 shipped, capping a same-day sprint of seven Quicks (006 through 012 = 16 commits to origin/master). Phase B production data with Anthropic Opus 4.6 + strict-tools + the new three-tier allowlist drove pass rate progression: gpt-4o-mini (0%) → gpt-4o-full local benchmark (70%) → Opus 4.6 + strict-tools alone (50%) → Quick 011 Tier 2 fallback (70%) → Quick 012 Tier 3 + chip auth-02 realignment (projected 90%+). The citation paraphrase problem is fully solved (Quick 009 strict-tools + Opus 4.6); the allowlist false-positive surface is closed (Quick 011 + 012); the chip vocabulary aligned with source (Quick 012). Operator action: `git pull` + `pnpm build` + restart on D:\kbroles to land Quick 012. Then re-fire chip auth-02 to confirm pass rate. Open code follow-ups (renumbered): **Quick 013 candidate** — provider logging on `chat_request_completed` event (observability gap from Phase B diagnosis); **Quick 014 candidate** — Bedrock prompt caching via `cache_control` (potential 10x cost reduction at 6125-token stable prompt); **Quick 015 candidate** — stochastic regression eval (chip stability test); **Quick 016 candidate** — GHA deploy.yml audit; **Quick 017 candidate** — text-mode markdown-fence stripping (escape-hatch-only); **Quick 018 candidate** — wire `chip_id` from client so `chip_vs_freeform` telemetry correctly tags chip-driven requests. Operator note: the chip text rewording should be mirrored back to `info/KB_Assistant_ClaudeCode_Handover.md §16` (untracked operator-managed doc); inline comment in `src/prompts/suggested.ts` auth-02 flags this for the next handover revision.*
