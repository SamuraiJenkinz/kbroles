...
*Last activity: 2026-05-11 — Quick 008 shipped. kbroles can now be switched between OpenAI/Azure-OpenAI and Anthropic Claude 4.5+ (via the MGTI `/coreapi/llm/anthropic/v1` proxy) by setting a single env var — no code change, no rebuild required. Backward compatible: every existing deploy continues to work unchanged. Three open operator-side follow-ups before live use: (1) provision MGTI Anthropic API key via the Hubble PR flow per the MGTI llm-anthropic spec; (2) pick the regional endpoint matching the deploy (NASA prod for D:\kbroles); (3) run a 10-trial benchmark against Claude Sonnet 4.5 on the failing Author chip to compare pass rate against the local gpt-4o 7/10 baseline from quick-006. Open code follow-ups remain unchanged from after quick-007: **Quick 009 candidate** — allowlist over-strictness; **Quick 010 candidate** — stochastic regression eval; **Quick 011 candidate** — GHA deploy.yml audit. The Critical Gap on the Anthropic path (no `tools` / strict-schema enforcement in the MGTI proxy spec) is the load-bearing risk worth monitoring once Anthropic mode goes live in production — if pass rate is materially worse than the gpt-4o (full) path, the lack of strict-schema backstop is the likely culprit.*
