...
*Last activity: 2026-05-02 — Quick task 005 shipped — `pnpm dev` restored to working state by forcing Webpack mode (Quick 003 inadvertently broke local dev under Turbopack). Surfaced during local diagnostic work on the failing Author chip "What fields do I need to fill in on the form?" — that investigation continues in Quick 006 (prompt verbatim-rule strengthening, +30pp pass-rate at the local 10-trial benchmark) and Quick 007 (the still-open allowlist-overstrictness finding that surfaced when paraphrase failures dropped). The original Open Diagnostic Loop from Quick 004 is partially closed: the failure mode is now identified as stochastic paraphrase (citation `quote_not_in_body`), not source/section misidentification — model picks the right SNOW_FORM/required-fields anchor, but ~50% of the time paraphrases the quote text instead of returning a verbatim substring.*
