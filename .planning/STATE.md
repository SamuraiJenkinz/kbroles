...
*Last activity: 2026-05-01 — Quick task 003 shipped — pilot workarounds replaced with code fixes. Pilot is functionally operational on the workaround path; redeploying the standalone bundle will pick up the proper fixes naturally. Three remaining tech-debt items from quick-001..003: (a) eval suite re-baseline against gpt-4o full or gpt-4.1-mini, (b) deploy.yml `cp -r public` step assumes a `public/` directory that doesn't exist in the repo, (c) `prompt_tokens`/`completion_tokens` returning null in chat telemetry on gpt-4.1-mini.*
