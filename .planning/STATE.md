...
*Last activity: 2026-05-12 — Quick 010 shipped. Live Phase A smoke test against the MGTI non-prod NASA proxy caught a URL-path bug in the Quick 008/009 adapter (missing `/messages` suffix) before any kbroles deploy. The smoke test workflow that 008-SUMMARY.md prescribed proved its worth — operator's first kbroles request under LLM_PROVIDER=anthropic would have 404'd in production without this fix. Path now matches the MGTI quickstart.md documentation. Confirmed end-to-end: API key valid, Bedrock guardrail engaged with no action, Claude Sonnet 4.6 returned valid response in tool-use mode, EU AWS region (`eu-west-1`) reachable. Three side observations from the smoke test logged in 010-SUMMARY.md: (1) Bedrock prompt-caching headers exist (cache write/read currently 0; capability available — meaningful cost-win lever if `cache_control` passes through); (2) Claude in text-mode wraps JSON in markdown code fences (only relevant if the operator ever flips ANTHROPIC_TOOLS_SUPPORTED=false; current default tool-use mode sidesteps this); (3) Bedrock guardrail telemetry is detailed and could feed kbroles logs for false-positive diagnosis. Next operator step: `git pull` on D:\kbroles + `pnpm build` + optionally update `.env.production` to flip `LLM_PROVIDER=anthropic` for Phase B kbroles staging test. Open code follow-ups (renumbered): **Quick 011** — allowlist over-strictness; **Quick 012** — stochastic regression eval; **Quick 013** — Bedrock prompt caching via `cache_control`; **Quick 014** — GHA deploy.yml audit; **Quick 015** — text-mode markdown-fence stripping (only if escape hatch ever needed).*
