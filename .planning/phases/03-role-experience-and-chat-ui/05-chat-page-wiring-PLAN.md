...
must_haves:
  truths:
    ...
    - "InputBar auto-focus after every send and after role-select transition (CONTEXT §Input bar)"
  ...
  artifacts:
    ...
    - path: "src/chat-ui/ChatSurface.tsx"
      provides: "'use client' main chat — Header + Greeting + MessageList + ChipRow + InputBar + ErrorCard; owns useReducer(chatReducer), useChatStream, useDraftBuffer, usePrompts, ChangeRoleDialog state"
      exports: ["ChatSurface"]
      min_lines: 140
...
<context>
Depends on: Plans 01 (scaffold), 02 (reducer + types + sourceTitles + time), 03 (hooks), 04 (components).
...
2. Dialog renders. User confirms (clicks "Change role" button).
...
1. User clicks Retry inside ErrorCard (which is rendered in place of the assistant bubble with state='error').
...
<task type="auto">
  ...
  <action>
    ...
    5. **Create `src/chat-ui/__tests__/usePrompts.test.tsx`** — `// @vitest-environment jsdom`:
       ...
       - role='consumer': fetch called with URL matching `/api/prompts?role=consumer`; response mock returns {role:'consumer', prompts: [5 fake chips]}; post-resolution chips.length === 5.
       - role='author': fetch URL contains `role=author`; chips.length === 8 (mock).
       ...
       - Role change from 'consumer' to 'author' issues a new fetch (fetch.mock.calls.length increases by 1); the prior fetch's AbortController.abort() was called (assert signal.aborted on the first call's signal argument).
  ...
  <done>
    ChatPage hydration gate prevents flash; role is persisted via sessionStorage; usePrompts fetches role-specific chips and degrades gracefully on failure; root page.tsx now delivers the live chat page.
  </done>
</task>

<task type="auto">
  <name>Task 5.2: ChatSurface — reducer + stream + dispatches + all flows + Pitfall-13 test</name>
  <files>src/chat-ui/ChatSurface.tsx, src/chat-ui/__tests__/ChatSurface.test.tsx</files>
  <action>
    1. **Create `src/chat-ui/ChatSurface.tsx`** — `'use client'`. This is the wiring hub. Reference structure (LOCKED — implementation is free to reorganise internals but must preserve flow ordering and dispatch mappings):
       ...
       export function ChatSurface({ role, onChangeRole }: { role: Role; onChangeRole: () => void }) {
         ...
         const dispatchSend = useCallback(
           ...
           [role, send, setDraft, clearDraft, state.messages],
         )
         ...
         const handleNewConversation = useCallback(() => {
           ...
           asstIdRef.current = null
           inputRef.current?.focus()
         }, [state.inFlightId, stop, clearDraft, setDraft])
       ...
       Adjust interfaces where needed:
       - **MessageList needs an `onRetry` prop** → plumb into Message, which renders `<ErrorCard onRetry={() => onRetry(message.id)} errorCode={...} requestId={...}/>` when `state==='error'`. Update MessageList + Message in this task (small targeted edit).
       - **InputBar needs ref forwarding** → wrap with `forwardRef<HTMLTextAreaElement, Props>`; most Plan-04 Task 4.2 implementations already use a ref internally — expose via forwardRef. Update InputBar in this task.

       Both edits are small and localised; keep them in this plan rather than reopening Plan 04.
    ...
    2. **Create `src/chat-ui/__tests__/ChatSurface.test.tsx`** — `// @vitest-environment jsdom`. Use @testing-library/react + user-event. Mock fetch for both /api/prompts and /api/chat:
       ...
       **Test 1 — Empty state → chip click → stream → done:**
       - Mock fetch: `/api/prompts?role=consumer` returns 5 fake chips; `/api/chat` returns an SSE stream that emits answer_delta('Hello'), citations([{source_id:'KB0022991', section_id:'flagging-articles', quote:'x'}]), done.
       - Render `<ChatSurface role='consumer' onChangeRole={vi.fn()} />`.
       - Wait for chip row to render (await findByRole('button', {name:/chip label 1/i})).
       - user.click(chip) → user message and assistant bubble appear (await findByText('Hello')).
       ...
       **Test 5 — Change role confirm (Pitfall 13 CRITICAL):**
       - Start a stream with role='consumer' that emits answer_delta('partial ') and delays the next event.
       - After the first delta renders, click the role pill → "Change role" option.
       - Dialog appears. Click confirm ("Change role" red button).
       - Assert in order:
       ...
       **Test 6 — Error bubble + Retry (CHAT-07):**
    ...
    3. **Commit:** `feat(phase-3/plan-05): wire ChatSurface + Pitfall-13 change-role ordering + retry flow with reconstructed user turn`.
  </action>
  <verify>
    - `pnpm typecheck` passes (expect edits to MessageList + Message + InputBar for the new props/ref).
    - `pnpm test` — ≥8 new ChatSurface/ChatPage tests green. Critical tests by name: "Pitfall 13" or "change role", "Stop preserves partial", "Retry", "Returning user". Total ≥324.
    - Manual smoke: `pnpm dev` → visit http://localhost:3000 → pick Consumer → see greeting + chips → click a chip → see streaming text from the real /api/chat (dev-mode OpenAI path) → console shows no errors → 'New conversation' resets → 'Change role' prompts confirm.
  ...
  <done>
    Full chat experience is wired end-to-end. ChatPage hydration gate + role routing + ChatSurface composition all deliver the 5 Phase-3 SCs behaviourally. Pitfall 13 ordering is test-enforced. Retry correctly reconstructs the user turn. The running app serves the Phase-3 MVP at http://localhost:3000.
  </done>
</task>
...
<verification>
  ...
  - `pnpm test` green — ≥14 new tests across this plan; total ≥324.
...
<success_criteria>
Phase-3 SC #1 — user lands on RoleSelect, picks consumer/author → ChatSurface with role-aware greeting + role-specific chips: Tests 1 + 8 (plus usePrompts tests from Task 5.1).
...
Phase-3 SC #3 — Stop / New conversation / Change role (with confirm): Tests 3 + 4 + 5.
...
- New test count delta (≥14) and total repo test count (≥324).
- Manual smoke notes: Consumer + Author role pick, chip-submit, Stop, New conversation, Change-role flow — all verified via `pnpm dev`.
